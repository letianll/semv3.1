...
use app\admin\model\ExtendField as ExtendFieldModel;
use app\core\lib\Json;
use app\core\lib\Util;
use app\core\lib\Upload;

class ExtendField extends Controller{
	...
	public function lists(){
		...
		return json([
			"code"=> 0,
			"msg"=> "成功获取数据",
			"count"=> count(ExtendFieldModel::lists($data)[1]),
			"data"=>ExtendFieldModel::lists($data)[0]
		]);
	}
	...
	public function listsExcel(){
		return json([
			"code"=> 0,
			"msg"=> "成功获取数据",
			"data"=>ExtendFieldModel::listsExcel()
		]);
	}
	/**
	 * add 扩展字段
	...
	public function add(){
		if(request()->isPost()){			
			$data = $this->request->param();
			return json( (new ExtendFieldModel())->add($data));	
		}				
	}
	...
	public function ajaxAddMulti(){
		if(request()->isPost()){			
			...
			if(isset($data['data'][0]['sheet1']) && count($data['data'][0]['sheet1'])>=2 ){
				return json( (new ExtendFieldModel())->ajaxAddMulti($data['data'][0]['sheet1']));
			}else{
	...
	public function edit(){
		if(request()->isPost()){			
			$data = $this->request->param();
			return json( (new ExtendFieldModel())->edit($data));	
		}				
	}
	...
	public function ajaxChange(){
		if(request()->isGet()){			
			$data = $this->request->param();
			return json( (new ExtendFieldModel())->ajaxChange($data));	
		}				
	}
	...
	public function delete(){
		...
		if(empty($data['id'])){
			...
			$res = (new ExtendFieldModel())->del($data['id']);
			return json($res);
		}
	}
