<?php /*a:1:{s:77:"D:\webroot\semclientcmsv3.1\application\admin\view\content\article\image.html";i:1585116023;}*/ ?>
<!DOCTYPE html>
...
<style>
	...
	.layui-tree-main{text-align: left;}
	.layui-tree-set{text-align:left;}
                    ...
                    <div class="layui-card-body clearfix image-box" style="padding: 10px;height: 360px;z-index:10;">
                        ...
                        <div class="empty-image" style="width: 100%;height: 100%;text-align: center;" v-if="!imageList.length && loading == false">
                            <div class="layui-img-box">
                                <img src="/static/admin/images/empty.jpg" style="height: 350px;" alt="" >
                            </div>
                        </div>
                    </div>
...
<script src="/static/admin/js/layuiList.js?id=dsfwef"></script>
<script>
    // var pid = 0,small = 0 ,parentinputname = 'slider_image';//
	var pid = 0,small = 0  ,parentinputname = '<?php echo htmlentities(app('request')->param('fodder')); ?>';//
    new Vue({
        el: "#app",
        data: {
            categoryList:[],
        ...
        methods:{
            ...
            moveCate:function(){
				var that=this;
                if(!this.selectImages.length) return;
                this.getOpenWindow('移动图片',this.U({a:'moveimg'})+'?imgaes='+this.selectImagesIDS);
				return that.getImageList();
            },
            //使用选中图片
            useImages:function(){
                if(!this.selectImages.length) return;
...
//                if(!this.searchTitle) return layList.msg('请输入搜索内容！');
            ...
            addCategory:function (item,pid) {
                ...
                return this.getOpenWindow(item.name ? item.name+'添加' : '新增分类',this.U({a:'addcate',q:{id:pid ==0 ? id : pid }}));
            },
            //修改分类
