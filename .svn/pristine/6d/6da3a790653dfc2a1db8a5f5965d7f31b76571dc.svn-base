...
class SystemAttachment extends BaseModel
{
    ...
    public static function getConfigValue($menu){
        if(empty($menu) || !($config_one = self::where('name', $menu)->find())) return false;
    ...
    public static function getImageList($where)
    {
        ...
        if(isset($where['pid']) && (int)$where['pid']){
			$model = $model->where('pid', $where['pid']);
			$count = self::where(['pid'=>$where['pid']])->count();
		}else{
			$count = self::count();
		} 
        ...
        $site_url = self::getConfigValue('site_url');
