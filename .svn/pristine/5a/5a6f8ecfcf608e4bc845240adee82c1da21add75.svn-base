...
class Product extends Common
{
    ...
    public function edit(){
        if($this->request->isGet()){
            $id = intval(input('param.id'));
            $data = db('ProductCategory')->where(['is_check'=>1,'id'=>$id])->find();
            $this->assign('data',$data);

            $cate_list = db('ProductCategory')->where(['is_check'=>1])->field('id,parentid,category')->order('paixu desc,id asc')->select();
